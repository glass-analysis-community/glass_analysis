...
try:
  opts, args = getopt.gnu_getopt(sys.argv[1:], "k:d:x:y:a:c:ih" +
                                               runset.shortopts +
                                               trajset.shortopts +
                                               prog.shortopts +
                                               frames.shortopts +
...
if ta != 0  or tc != 0:
  tbval_bins = np.zeros((2, size_fft, size_fft, size_fft), dtype=np.float64)
else:
  tbval_bins = np.zeros((1, size_fft, size_fft, size_fft), dtype=np.float64)
...
if ta - tc != 0:
  ...
  obsv_s = np.empty((4, runset.n_runs, size_fft, size_fft, size_fft), dtype=np.float64)
else:
  # 0 - G(r,t_1,t_3) - Fourier transformed particle positions between
  #     starts of first and second intervals, total part (X3)
  # 1 - G4_d(r,t_1,t_2,t_3,t_4) - G4 distinct part (X6)
  obsv_s = np.empty((2, runset.n_runs, size_fft, size_fft, size_fft), dtype=np.float64)

# Jackknife and full means of observables and combined observables not
# associated with a spatial vector. In first dimension:
if ta - tc != 0:
  # 0 - C(t_1,t_2) (X1)
  # 1 - C(t_3,t_4) (X2)
...
# Jackknife and full means of observables and combined observables that
...
if ta - tc != 0:
  ...
  # 5 - C(t_1,t_2) * G(r,t_1,t_3) (X1 * X3)
  # 6 - C(t_3,t_4) * G(r,t_1,t_3) (X2 * X3)
  mobsv_s = np.empty((7, size_fft, size_fft, size_fft), dtype=np.float64)
  jmobsv_s = np.empty((7, size_fft, size_fft, size_fft), dtype=np.float64)
else:
  # 0 - G(r,t_1,t_3) (X3)
  # 1 - G_d(r,t_1,t_3) (X4)
  # 2 - G4_d(r,t_1,t_2,t_3,t_4) (X6)
  # 3 - G_d(r,t_1,t_3)^2 (X3^2)
  # 4 - G_d(r,t_1,t_3) * G4_d(r,t_1,t_2,t_3,t_4) (X4 * X6)
  # 5 - C(t_1,t_2) * G(r,t_1,t_3), C(t_3,t_4) * G(r,t_1,t_3) (X1 * X3,
  #     X2 * X3)
  mobsv_s = np.empty((6, size_fft, size_fft, size_fft), dtype=np.float64)
  jmobsv_s = np.empty((6, size_fft, size_fft, size_fft), dtype=np.float64)
...
if ta - tc != 0:
  ...
  if qshell_active == True:
    ...
    s4_comp = np.empty((4, size_fft, size_fft, (size_fft // 2) + 1), dtype=np.float64)
    s4_comp_std = np.empty((4, size_fft, size_fft, (size_fft // 2) + 1), dtype=np.float64)
    js4_comp = np.empty((4, size_fft, size_fft, (size_fft // 2) + 1), dtype=np.float64)
else:
  # 0 - S4^st - S4 initial structure contribution
  # 1 - S4^cr - S4 collective relaxation contribution
  # 2 - S4^mc - S4 mixed collective contribution
  # 3 - S4^st - S4 initial structure contribution standard deviation
  # 4 - S4^cr - S4 collective relaxation contribution standard deviation
  ...
  if qshell_active == True:
    ...
    s4_comp = np.empty((3, size_fft, size_fft, (size_fft // 2) + 1), dtype=np.float64)
    s4_comp_std = np.empty((3, size_fft, size_fft, (size_fft // 2) + 1), dtype=np.float64)
    js4_comp = np.empty((3, size_fft, size_fft, (size_fft // 2) + 1), dtype=np.float64)
  # Separate quantities for single particle contribution, as it is not
  # associated with a spatial vector when ta = -tc
  s4_sp_comp = 0.0
  s4_sp_comp_std = 0.0
  js4_sp_comp = 0.0
...
for index, tb in enumerate(tbvals):
  ...
  for i in range(0, runset.n_runs):
    ...
    for j in np.arange(0, initend - frames.start, framediff):
      ...
      # If needed, get particle coordinates and calculate w values for
      # second interval
      if ta != 0 or tc != 0:
        if ta - tc == 0:
          wcalc.calculate_w(w[1], j + ta - tc, x0, y0, z0, j + ta + tb, x1, y1, z1, i)
        else:
      ...
      tbval_bins[0] = np.histogramdd((x0i, y0i, z0i), bins=size_fft, range=((-0.5, size_fft - 0.5), ) * 3)[0]
      if ta - tc != 0:
        tbval_bins[1] = np.histogramdd((x2i, y2i, z2i), bins=size_fft, range=((-0.5, size_fft - 0.5), ) * 3)[0]
      ...
      tbval_bins[0] = np.histogramdd((x0i, y0i, z0i), bins=size_fft, range=((-0.5, size_fft - 0.5), ) * 3, weights=w[0])[0]
      if ta - tc != 0:
        tbval_bins[1] = np.histogramdd((x2i, y2i, z2i), bins=size_fft, range=((-0.5, size_fft - 0.5), ) * 3, weights=w[1])[0]
      elif ta != 0 or tc != 0:
        tbval_bins[1] = np.histogramdd((x0i, y0i, z0i), bins=size_fft, range=((-0.5, size_fft - 0.5), ) * 3, weights=w[1])[0]
      if ta != 0 or tc != 0:
        tbval_bins[0] = fft.irfftn(fft.rfftn(tbval_bins[0]) * np.conjugate(fft.rfftn(tbval_bins[1])), s=tbval_bins.shape[1:])
      else:
        # Uses np.abs(), which calculates norm of complex numbers
  ...
  if qshell_active == True:
    ...
    if ta - tc != 0:
      ...
      # est_65d4 must be added at position [0, 0, 0]. This has the same
      # effect as adding this to all qshell-averaged elements after the
      # FFT, as the FFT coefficient is always 1 at this position.
  ...
  if qshell_active == True:
    ...
    #      quantities
    # 13 - frame difference corresponding to t_b
    for i in range(0, s4_discrete.shape[-1]):
      outfile.write("%f %f %d %f %f %f %f %f %f %f %f %d %d\n"
                    %(time_tb,
                      ...
                      s4_discrete[1][i],
                      s4_discrete[2][i],
                      s4_discrete[3][i] if ta - tc != 0 else s4_sp_comp,
                      s4_discrete_std[0][i],
                      s4_discrete_std[1][i],
                      s4_discrete_std[2][i],
                      s4_discrete_std[3][i] if ta - tc != 0 else s4_sp_comp_std,
                      norm,
                      tb))
    ...
    for i in range(0, s4_shells.shape[-1]):
      outfile.write("%f %f %d %f %f %f %f %f %f %f %f %d %d\n"
                    %(time_tb,
                      ...
                      s4_shells[1][i],
                      s4_shells[2][i],
                      s4_shells[3][i] if ta - tc != 0 else s4_sp_comp,
                      s4_shells_std[0][i],
                      s4_shells_std[1][i],
                      s4_shells_std[2][i],
                      s4_shells_std[3][i] if ta - tc != 0 else s4_sp_comp_std,
                      norm,
                      tb))
    ...
    for i in range(0, size_fft):
      for j in range(0, size_fft):
        for k in range(0, (size_fft // 2) + 1):
          ...
          # 14 - frame difference corresponding to t_b
          outfile.write("%f %f %f %f %f %f %f %f %f %f %f %f %d %d\n"
                        %(time_tb,
                          ...
                          s4_comp[1][i][j][k],
                          s4_comp[2][i][j][k],
                          s4_comp[3][i][j][k] if ta - tc != 0 else s4_sp_comp,
                          s4_comp_std[0][i][j][k],
                          s4_comp_std[1][i][j][k],
                          s4_comp_std[2][i][j][k],
                          s4_comp_std[3][i][j][k] if ta - tc != 0 else s4_sp_comp_std,
                          norm,
                          tb))
